...
import { seedObligations } from './governance'
import type {
  AgendaItem,
  Attendance,
  Obligation,
...
export class SupabaseMeetingRepository implements MeetingRepository {
  ...
  async load(): Promise<MeetingsData> {
    ...
    const [meetings, agenda, attendance, motions, seats, settings, me, reports, versions, people, bodies, obligations] = await Promise.all([
    ...
    for (const [what, result] of [['read meetings', meetings], ['read the agenda', agenda], ['read attendance', attendance], ['read motions', motions], ['read the roll', seats], ['read reports', reports], ['read versions', versions], ['read names', people], ['read the year', obligations]] as const) {
