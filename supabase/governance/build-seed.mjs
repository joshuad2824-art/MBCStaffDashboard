...
/* Turns the transcribed governance corpus into SQL for governance_document and
   ...
   How a file is read:
     ...
     - the audience is the front matter's `audience` — a comma-separated or
       bracketed list of body slugs, where the word `deacon` means the whole
       deacon side (the Board, the Body and the four standing committees) and
       `staff` means the staff body. A file with no `audience` key is addressed
       to the deacon side and nobody else; nothing reaches the staff unless the
       corpus says so, file by file (migration 0014). A word that names no body
       stops the run, because the trigger on the table would refuse it anyway.
...
/* The bodies 0004 seeds, which is what an audience may name. `deacon` is the
   deacon side as migration 0014 defaults it — everything that is not staff. */
const DEACON_SIDE = ['deacon-board', 'deacon-body', 'committee:finance', 'committee:personnel', 'committee:building-grounds', 'committee:family-assistance']
const BODIES = new Set(['staff', ...DEACON_SIDE])

/** `audience: staff, deacon` / `audience: [staff, committee:finance]` → body slugs, in order, without repeats. */
function readAudience(value, rel) {
  const words = String(value ?? '').replace(/^\[|\]$/g, '').split(/[,\s]+/).map((w) => w.trim().replace(/^["']|["']$/g, '')).filter(Boolean)
  if (words.length === 0) return DEACON_SIDE
  const slugs = []
  for (const word of words) {
    const expanded = word.toLowerCase() === 'deacon' ? DEACON_SIDE : [word]
    for (const slug of expanded) {
      if (!BODIES.has(slug)) {
        console.error(`${rel}: audience names no body: ${slug}`)
        process.exit(1)
      }
...
for (const path of files) {
  ...
  const audience = readAudience(fields.audience, rel)
  documents.push({ slug, kind, code, title, body: text.trim(), rel, folder, audience })
}
...
documents.forEach((d, index) => {
  out.push(
    `insert into governance_document (slug, kind, code, title, body, position, audience) values (${q(d.slug)}, ${q(d.kind)}, ${q(d.code)}, ${q(d.title)}, ${q(d.body)}, ${(index + 1) * 10}, ${arr(d.audience)})` +
      ` on conflict (slug) do update set kind = excluded.kind, code = excluded.code, title = excluded.title, body = excluded.body, position = excluded.position, audience = excluded.audience, updated_at = now();`,
  )
})
...
const toStaff = documents.filter((d) => d.audience.includes('staff'))
console.error(`${documents.length} documents (${toStaff.length} addressed to the staff), ${findings.length} findings`)
if (toStaff.length) console.error('addressed to the staff (audience in front matter):\n  ' + toStaff.map((d) => d.rel).join('\n  '))
