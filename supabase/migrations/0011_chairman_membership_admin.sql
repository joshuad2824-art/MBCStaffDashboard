-- The Board chairman may maintain Board and Board-committee seats from the
-- application. Membership remains a separate decision from account access:
-- seat somebody first, then grant limited access from People when needed.
...
begin
  ...
  select p.id,
    ...
    left join body b
     ...
     and (
       b.slug = 'deacon-board'
       or b.parent_body_id = (select id from body where slug = 'deacon-board')
     )
   where p.active
...
begin
  ...
  if target_active then
    ...
    update membership
       set active = false,
           term_end = case
             -- A future seat can be withdrawn without creating an impossible
             -- end-before-start term. For a seat that has begun, preserve the
             -- historical end date or close it today, whichever comes first.
