...
-- is open to everyone who signs in — 0014 gave each document an audience and
-- 0015 took it away again, on Joshua's decision that the bylaws, policies and
-- procedures are the church's own documents and need no shelf kept from
-- either side. The docket has no such standing: it stays the deacon side's.
...
select test.assert((select count(*) from governance_finding) = 1,  'grounds chair: reads the docket — a committee seat is on the deacon side');
reset role;

-- 10c. The staff side reads the whole manual too, and never the docket.
...
update governance_document set title = 'Renamed' where slug = 'a009';
reset role;
select test.assert((select title from governance_document where slug = 'a009') like 'A009%', 'staff: an update aimed at the manual changed nothing');
